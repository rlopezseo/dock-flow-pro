...
const config: ProductPageConfig = {
  ...
  hero: {
    ...
    subtext: "Backhaul matching that fills return trips automatically. Cut deadhead miles by 45%, lower transport costs, and reduce your carbon footprint. No phone calls required.",
  ...
  outcomes: {
    ...
    heroCard: {
      ...
      description: "The system matches return trips with available loads based on delivery point, timing, equipment type, and driver HOS. Trucks that would drive home empty get filled automatically.",
    },
    cards: [
  ...
  capabilities: {
    ...
    headline: "Backhaul optimization software features: matching, network analysis, and sustainability",
    groups: [
  ...
  useCases: {
    label: "Use Cases",
    headline: "Backhaul optimization for fleets, 3PLs, retail, and manufacturing",
    cases: [
      { id: "fleet", icon: Truck, label: "Private Fleets", image: imgManufacturing, headline: "Backhaul optimization for company-owned fleets", description: "Your trucks deliver outbound loads and drive home empty. The system finds return loads that fit your schedule, equipment, and driver availability. Every return trip becomes revenue.", benefits: ["Return load matching for owned trucks", "Driver HOS compliance checking", "Equipment type filtering", "Revenue per trip optimization"] },
      { id: "3pl", icon: Package, label: "3PL & Brokers", image: img3pl, headline: "Backhaul matching to keep carriers' trucks full", description: "Offer your carriers backhaul opportunities that keep their trucks loaded. Better carrier retention, lower rates, and margin on both legs.", benefits: ["Carrier backhaul portal", "Both-leg margin management", "Carrier satisfaction improvement", "Network-wide optimization"] },
  ...
  howItWorks: {
    ...
    steps: [
      { icon: MapPin, step: "01", title: "Truck Completes Delivery", description: "When your truck delivers its load, the system immediately scans for available return loads near the delivery point." },
      ...
      { icon: ScanLine, step: "04", title: "Execute & Track", description: "The backhaul load is tracked in real time like any other shipment. Full visibility from pickup to delivery." },
  ...
  faq: {
    ...
    items: [
      { q: "What is backhaul optimization software?", a: "Backhaul optimization software automatically finds freight loads for trucks that would otherwise drive empty on their return trip. It matches available loads with returning trucks based on location, timing, and equipment, eliminating deadhead miles and generating revenue from return trips." },
      { q: "How fast does it find a return load?", a: "TrucksOnTheMap identifies and scores the best backhaul option in 12 seconds on average. It evaluates proximity, timing, equipment requirements, driver hours-of-service, and profitability across thousands of available loads." },
      { q: "What is the difference between backhaul matching and triangulation?", a: "Backhaul matching finds a direct return load from your delivery point back toward your origin. Triangulation involves picking up a load that isn't directly on your return route but reduces overall empty miles when combined with other loads. The software handles both." },
      { q: "Does it check driver hours-of-service?", a: "Yes. Every match is validated against the driver's remaining HOS before being suggested. The system won't recommend a load that would cause an HOS violation." },
      { q: "Can it prioritize loads from my own network?", a: "Yes. The system cross-references your inbound and outbound freight first, finding internal backhaul opportunities within your own network before looking at external marketplace loads." },
      { q: "How does it help with ESG and sustainability?", a: "Every match includes a CO₂ savings calculation based on miles saved. The platform generates ESG-ready reports showing empty mile reduction, carbon savings, and fleet efficiency improvements for corporate disclosures." },
      { q: "What fill rates can it achieve?", a: "Customers typically reach return trip fill rates of 75–85%, up from an industry average of 40–50% without optimization. Results depend on your network density, lane structure, equipment types, and geographic coverage." },
      { q: "Does it work for cross-border freight?", a: "Yes. Cross-border backhaul matching is supported with customs documentation requirements, multi-currency rate management, and transit time calculations that account for border crossing delays." },
      ...
      { q: "How is it different from a load board?", a: "Load boards are passive: you search and hope. Backhaul optimization is proactive: the system identifies the best return load for each truck based on location, timing, HOS, and profitability, then notifies the driver." },
    ],
  },
  ctaFinal: {
